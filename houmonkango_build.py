...
def build_station_record(row, specialty_data):
    ...
    building = safe_str(row.get("方書（ビル名等）"))
    # 👑 修正済み（2026-07-14）：buildingがNone（方書欄が空欄）の場合、
    # 以前は str(None) 相当の "nan" が住所末尾に混入する不具合があった。
    # safe_str()でNoneを正しく判定するようにしたため、この不具合は解消済み。
    full_address = f"{address} {building}".strip() if building else address
